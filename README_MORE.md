...
| 是否在不同选项卡之间共享         | 否:x:                | 否:x:                | 是 :white_check_mark: | 是:white_check_mark: | 是:white_check_mark: |
| 是否在不同浏览器客户端之间共享   | 否:x:                | 否:x:                | 否:x:                 | 是:white_check_mark: | 否:x:                |
| 是否在服务器重启后保留数据       | 否:x:                | 否:x:                | 否:x:                 | 是:white_check_mark: | 否:x:                |
| 是否在页面重载后保留数据         | 是:white_check_mark: | 否:x:                | 是:white_check_mark:  | 是:white_check_mark: | 是:white_check_mark: |
| 是否只能用在ui.page内            | 是:white_check_mark: | 是:white_check_mark: | 是:white_check_mark:  | 否:x:                | 是:white_check_mark: |
| 是否需要客户端建立连接           | 是:white_check_mark: | 否:x:                | 否:x:                 | 否:x:                | 否:x:                |
| 是否只能在响应之前写入           | 否:x:                | 否:x:                | 否:x:                 | 否:x:                | 是:white_check_mark: |
| 是否要求数据可序列化             | 否:x:                | 否:x:                | 是:white_check_mark:  | 是:white_check_mark: | 是:white_check_mark: |
| 是否需要设置`storage_secret`参数 | 否:x:                | 否:x:                | 是:white_check_mark:  | 否:x:                | 是:white_check_mark: |
